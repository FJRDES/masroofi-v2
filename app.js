...
function renderHome(){
  ...
  $('todayLimitText').textContent=`${dayLabel} · ${formatDisplayDate(tkey)} · الحد اليومي: ${money(state.settings.dailyLimit)}`;
  $('weekLimitText').textContent=`الأسبوع ${fmtNum(weekNumberInBudgetCycle(wr.start))} من دورة الميزانية · الحد الأسبوعي: ${money(state.settings.weeklyLimit)} · ${compactDateRange(wr.start, wr.end)}`;
  $('monthLimitText').textContent=`دورة الميزانية: ${budgetCycleLabel()} · الحد الشهري: ${money(budget)}`;
