...
const state = { user:null, expenses:[], settings:{dailyLimit:0,weeklyLimit:0,monthlyBudget:0,currency:'SAR',budgetCycleStart:'',budgetCycleEnd:''}, unsub:[], quickDetail:null, analytics:{} };
...
function weekNumberInBudgetCycle(weekStart){
  const br = budgetCycleRange();
  const base = new Date(br.start); base.setHours(0,0,0,0);
  const ws = new Date(weekStart); ws.setHours(0,0,0,0);
  const diffDays = Math.floor((ws - base) / 86400000);
...
document.querySelectorAll('[data-analytics-filter]').forEach(el=>{
  const open = ()=>openAnalyticsFilter(el.dataset.analyticsFilter);
...
function getQuickDetailData(type){
  ...
  if(type==='today'){
    ...
    const br=budgetCycleRange();
    title='عمليات شهر الميزانية'; list=state.expenses.filter(e=>inRange(e,br.start,br.end)); chartTitle='مصروفات شهر الميزانية حسب الأيام'; filename='masroofi_budget_month.pdf'; group='month';
  } else if(type==='cycle'){
    const br=budgetCycleRange();
...
function quickChartData(detail){
  ...
  if(detail.group==='month') return groupByDate(detail.list);
...
function cycleElapsedDays(range=budgetCycleRange()){
  const start = new Date(range.start); start.setHours(0,0,0,0);
  const end = new Date(range.end); end.setHours(0,0,0,0);
  const today = toDateObj(todayISO(),'00:00');
  const capped = today < start ? start : (today > end ? end : today);
  return Math.max(1, Math.floor((capped - start) / 86400000) + 1);
}
function cycleTotalDays(range=budgetCycleRange()){
  const start = new Date(range.start); start.setHours(0,0,0,0);
  const end = new Date(range.end); end.setHours(0,0,0,0);
...
function topByField(list, field){
  ...
  list.forEach(e=>{
    ...
    if(!key) return;
    const item = map.get(key) || {name:key,total:0,count:0,list:[]};
    item.total += Number(e.amount||0);
    item.count += 1;
    item.list.push(e);
    map.set(key,item);
  });
  return [...map.values()].sort((a,b)=>b.total-a.total || b.count-a.count || a.name.localeCompare(b.name))[0] || null;
}
function topDayInList(list){
  const map = new Map();
  list.forEach(e=>{
    const key = normalizeDateValue(e.date);
    if(!key) return;
    const item = map.get(key) || {date:key,total:0,count:0,list:[]};
    item.total += Number(e.amount||0);
    item.count += 1;
    item.list.push(e);
    map.set(key,item);
  });
  return [...map.values()].sort((a,b)=>b.total-a.total || a.date.localeCompare(b.date))[0] || null;
}
function largestExpense(list){
  return [...list].sort((a,b)=>Number(b.amount||0)-Number(a.amount||0))[0] || null;
}
function analyticsData(){
  const range = budgetCycleRange();
  const list = state.expenses.filter(e=>inRange(e,range.start,range.end));
  const used = sum(list);
  const budget = Number(state.settings.monthlyBudget||0);
  const remaining = budget - used;
  const elapsed = cycleElapsedDays(range);
  const totalDays = cycleTotalDays(range);
  const left = daysLeftInCycle();
  const avgDaily = used / Math.max(elapsed,1);
  const avgTransaction = list.length ? used / list.length : 0;
  const topCategory = topByField(list,'category');
  const topBeneficiary = topByField(list,'beneficiary');
  const topPlace = topByField(list,'place');
  const topDay = topDayInList(list);
  const largest = largestExpense(list);
  let forecastTitle = 'لا توجد بيانات كافية';
  let forecastMeta = 'يعتمد على متوسط الصرف اليومي';
  if(budget > 0 && used > 0 && avgDaily > 0){
    if(remaining <= 0){
      forecastTitle = 'تم تجاوز الميزانية';
      forecastMeta = `تجاوز بمقدار ${money(Math.abs(remaining))}`;
    } else {
      const daysToDeplete = Math.floor(remaining / avgDaily);
      if(daysToDeplete < left){
        forecastTitle = `قد تنفد بعد ${fmtNum(Math.max(daysToDeplete,0))} يوم`;
        forecastMeta = `المتبقي الحالي ${money(remaining)}`;
      } else {
        forecastTitle = 'تكفي حتى نهاية الدورة';
        forecastMeta = `المتبقي الحالي ${money(remaining)}`;
      }
    }
  }
  const budgetRatio = budget > 0 ? used / budget : 0;
  const timeRatio = elapsed / totalDays;
  let health = {className:'neutral', title:'لا توجد بيانات كافية', note:'اضبط دورة الميزانية وأضف عمليات داخلها.'};
  if(budget > 0 && list.length){
    const gap = budgetRatio - timeRatio;
    if(budgetRatio >= 1) health = {className:'bad', title:'إنفاق مرتفع', note:`تم استهلاك ${percentText(budgetRatio)} من الميزانية.`};
    else if(gap > .25) health = {className:'bad', title:'يحتاج انتباه', note:`الصرف أسرع من تقدم الدورة بـ ${percentText(gap)}.`};
    else if(gap > .10) health = {className:'warn', title:'راقب الصرف', note:`معدل الصرف أعلى من الوقت المنقضي.`};
    else health = {className:'ok', title:'ضمن المعدل', note:`الصرف متوازن مع أيام الدورة.`};
  }
  return {range,list,used,budget,remaining,elapsed,totalDays,left,avgDaily,avgTransaction,topCategory,topBeneficiary,topPlace,topDay,largest,forecastTitle,forecastMeta,health};
}
...
function renderAnalytics(){
  ...
  state.analytics = a;
  setText('analyticsCycleLabel', `دورة ${budgetCycleLabel()} · ${fmtNum(a.list.length)} عملية`);
  const health = $('cycleHealth');
  if(health){
    health.classList.remove('ok','warn','bad','neutral');
    health.classList.add(a.health.className);
  }
  setText('cycleHealthTitle', a.health.title);
  setText('cycleHealthNote', a.health.note);

  const total = Math.max(a.used,1);
  function setTop(prefix, item){
    setText(`${prefix}Name`, item ? item.name : 'لا يوجد');
    setText(`${prefix}Meta`, item ? `${fmtNum(item.count)} عملية · ${money(item.total)} · ${percentText(item.total/total)}` : `0 عملية · ${money(0)}`);
  }
  setTop('topCategory', a.topCategory);
  setTop('topBeneficiary', a.topBeneficiary);
  setTop('topPlace', a.topPlace);
  setText('avgDailySpend', money(a.avgDaily));
  setText('avgDailyMeta', `${fmtNum(a.elapsed)} من ${fmtNum(a.totalDays)} يوم منقضي`);
  if(a.topDay){
    setText('topDayName', `${dayNameFromDate(a.topDay.date)} · ${formatDisplayDate(a.topDay.date)}`);
    setText('topDayMeta', `${money(a.topDay.total)} · ${fmtNum(a.topDay.count)} عملية`);
  } else {
    setText('topDayName', 'لا يوجد');
    setText('topDayMeta', money(0));
  }
  setText('cycleTransactionsCount', fmtNum(a.list.length));
  setText('cycleTransactionsMeta', `متوسط العملية ${money(a.avgTransaction)}`);
  if(a.largest){
    setText('largestExpenseAmount', money(a.largest.amount));
    setText('largestExpenseMeta', `${a.largest.category||'-'} · ${formatDisplayDate(normalizeDateValue(a.largest.date))}`);
  } else {
    setText('largestExpenseAmount', money(0));
    setText('largestExpenseMeta', 'لا يوجد');
  }
  setText('budgetForecastTitle', a.forecastTitle);
  setText('budgetForecastMeta', a.forecastMeta);
}
function openAnalyticsFilter(type){
  if(!state.user){ toast('سجل الدخول أولاً'); return; }
  const a = state.analytics?.list ? state.analytics : analyticsData();
  const cycleStart = a.range.startIso;
  const cycleEnd = a.range.endIso;
  const setChecked=(id,val)=>{ const el=$(id); if(el) el.checked=val; };
  if($('filterFrom')) $('filterFrom').value = cycleStart;
  if($('filterTo')) $('filterTo').value = cycleEnd;
  setChecked('useDateFilter', true);
  setChecked('searchCategory', true); setChecked('searchBeneficiary', true); setChecked('searchPlace', true); setChecked('searchNotes', false);
  let text='';
  if(type==='category' && a.topCategory){ text=a.topCategory.name; setChecked('searchCategory', true); setChecked('searchBeneficiary', false); setChecked('searchPlace', false); }
  else if(type==='beneficiary' && a.topBeneficiary){ text=a.topBeneficiary.name; setChecked('searchCategory', false); setChecked('searchBeneficiary', true); setChecked('searchPlace', false); }
  else if(type==='place' && a.topPlace){ text=a.topPlace.name; setChecked('searchCategory', false); setChecked('searchBeneficiary', false); setChecked('searchPlace', true); }
  else if(type==='day' && a.topDay){ if($('filterFrom')) $('filterFrom').value=a.topDay.date; if($('filterTo')) $('filterTo').value=a.topDay.date; }
  else if(type==='largest' && a.largest){ text=a.largest.category || a.largest.beneficiary || a.largest.place || ''; }
  if($('filterText')) $('filterText').value=text;
  nav('transactions');
  renderTransactions();
  bindItems();
}
...
function renderHome(){
  ...
  // مصروف الشهر هنا يتبع دورة الميزانية المحددة في الإعدادات، وليس الشهر الميلادي.
  const month=cycleList;
  ...
  const budgetRatio = budget ? cycleUsed / budget : 0;
  $('budgetNote').textContent = budget ? `المستخدم من الميزانية: ${money(cycleUsed)} (${percentText(budgetRatio)}) · الأيام المتبقية: ${fmtNum(daysLeftInCycle())} · دورة ${budgetCycleLabel()}` : 'اضبط ميزانية الدورة من الإعدادات';
  setBudgetProgress(cycleUsed, budget);
  const dayLabel = dayNameFromDate(tkey);
  $('todayLimitText').textContent=`${dayLabel} · ${formatDisplayDate(tkey)} · الحد اليومي: ${money(state.settings.dailyLimit)} · استهلاك: ${percentText(limitRatio(st,state.settings.dailyLimit))}`;
  $('weekLimitText').textContent=`الأسبوع ${fmtNum(weekNumberInBudgetCycle(wr.start))} من دورة الميزانية · الحد الأسبوعي: ${money(state.settings.weeklyLimit)} · استهلاك: ${percentText(limitRatio(sw,state.settings.weeklyLimit))} · ${compactDateRange(wr.start, wr.end)}`;
  $('monthLimitText').textContent=`دورة الميزانية: ${budgetCycleLabel()} · الحد الشهري: ${money(budget)} · استهلاك: ${percentText(limitRatio(sm,budget))}`;
...
function limitRatio(total,limit){ limit=Number(limit||0); return limit>0 ? Number(total||0)/limit : 0; }
function percentText(r){ if(!isFinite(r)||r<0) r=0; return `${fmtNum(Math.round(r*1000)/10)}%`; }
function usageColor(r){
  if(!isFinite(r)||r<0) r=0;
  const x=Math.min(r,1.15);
  if(x<=0.5) return '#16a34a';
  const t=Math.min((x-0.5)/0.5,1);
  const hue=Math.round(120*(1-t));
  return `hsl(${hue}, 78%, 42%)`;
}
function ensureProgress(el){
  let bar=el.querySelector('.usage-progress');
  if(!bar){
    bar=document.createElement('div');
    bar.className='usage-progress';
    bar.innerHTML='<span></span>';
    el.appendChild(bar);
  }
  return bar.querySelector('span');
}
function setCardStatus(el,total,limit){
  el.classList.remove('ok','warn','bad','neutral');
  const bar=ensureProgress(el);
  const r=limitRatio(total,limit);
  const width=Math.max(0,Math.min(r,1))*100;
  if(!Number(limit||0)){
    el.classList.add('neutral');
    el.style.removeProperty('--usage-color');
    bar.style.width='0%';
    bar.style.background='#94a3b8';
    return;
  }
  const color=usageColor(r);
  el.style.setProperty('--usage-color', color);
  bar.style.width=`${width}%`;
  bar.style.background=color;
  el.classList.add('usage');
}
function setBudgetProgress(total,limit){
  const hero=document.querySelector('.hero-card > div:first-child');
  if(!hero) return;
  let wrap=hero.querySelector('#budgetProgress');
  if(!wrap){
    wrap=document.createElement('div');
    wrap.id='budgetProgress';
    wrap.className='budget-progress usage-progress';
    wrap.innerHTML='<span></span>';
    hero.appendChild(wrap);
  }
  const r=limitRatio(total,limit);
  const color=Number(limit||0)?usageColor(r):'rgba(255,255,255,.7)';
  const fill=wrap.querySelector('span');
  fill.style.width=`${Math.max(0,Math.min(r,1))*100}%`;
...
function renderCharts(){ const now=new Date(), wr=weekRange(now), br=budgetCycleRange(); const today=state.expenses.filter(e=>normalizeDateValue(e.date)===todayISO()), week=state.expenses.filter(e=>inRange(e,wr.start,wr.end)), month=state.expenses.filter(e=>inRange(e,br.start,br.end)); let g=groupByHour(today); drawLine($('chartToday'),g.labels,g.vals); g=groupByWeekDays(week,wr); drawLine($('chartWeek'),g.labels,g.vals); g=groupByDate(month); drawLine($('chartMonth'),g.labels,g.vals); }
...
document.querySelectorAll('[data-report]').forEach(btn=>btn.onclick=()=>{
  ...
  if(type==='monthly'){title='تقرير شهر الميزانية'; const r=budgetCycleRange(); list=state.expenses.filter(e=>inRange(e,r.start,r.end)); chartMode='date'; chartTitle='مصروفات شهر الميزانية حسب الأيام';}
